...
class NormalizedS275FieldCalculator(DbConnection):
    def fill_calculated_fields(self):
        with Session(self._engine) as session:
            self._fill_employee_rollup_info(session)
            self._fill_private_assignments_values(session)
            session.commit()

    def _fill_employee_rollup_info(self, session):
    ...
    def _fill_private_assignments_values(self, session):
...
def main():
    parser = argparse.ArgumentParser(
        description='Populates calculated fields in the s275 db')
