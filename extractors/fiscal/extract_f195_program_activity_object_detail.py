#!python3
"""Walk data/fiscal/fiscal/, parse each F-195 Budget PDF's per-program
`OBJECTS OF EXPENDITURE` sub-report (GF9-XX).

Usage:
    python3 -m extractors.fiscal.extract_f195_program_activity_object_detail \\
        data/fiscal/fiscal/ --format csv \\
        > out_fiscal/fiscal_f195_program_activity_object_detail.csv
"""
...
def main():
    run_extract(
        parse_fn_module="extractors.fiscal.parsers.f195_program_activity_object_detail",
        parse_fn_name="parse_f195_program_activity_object_detail_pdf",
