# extractors/fiscal -- coverage gaps and form quirks

This file documents the known holes and quirks in the `fiscal_*` fact
tables: which OSPI sub-reports aren't captured (and why), where the
captured tables have residual gaps, and which printed-form oddities
the parsers absorb so a future debugger isn't surprised by them. For
table schemas and join keys see [CSV_GUIDE.md](CSV_GUIDE.md); for
genuinely open build-pipeline work see [TODO.md](TODO.md).
...
## Sub-reports not parsed (and why)
