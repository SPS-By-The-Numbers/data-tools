...
--              4 MLL / bilingual (programs 64 federal + 65 state)
WITH re AS (
...
SELECT
  ...
  CASE WHEN t.total_fte > 0 THEN
    CASE GREATEST(t.total_fte - t.sped_fte - t.t1_fte - t.lap_fte - t.bil_fte,
                  t.lap_fte, t.t1_fte, t.sped_fte, t.bil_fte)
      WHEN t.total_fte - t.sped_fte - t.t1_fte - t.lap_fte - t.bil_fte THEN 0
      WHEN t.lap_fte  THEN 1
      WHEN t.t1_fte   THEN 2
      WHEN t.sped_fte THEN 3
      ELSE 4 END
  ELSE
    CASE GREATEST(t.n_asgn - t.n_sped - t.n_t1 - t.n_lap - t.n_bil,
                  t.n_lap, t.n_t1, t.n_sped, t.n_bil)
      WHEN t.n_asgn - t.n_sped - t.n_t1 - t.n_lap - t.n_bil THEN 0
      WHEN t.n_lap THEN 1
      WHEN t.n_t1  THEN 2
      WHEN t.n_sped THEN 3
      ELSE 4 END
  END                                                AS pgroup
FROM pick p
