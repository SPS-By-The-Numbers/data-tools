...
(async () => {
  ...
  // Track page XHR inflight count by hooking the page's own ajax events.
  let pageXhrInflight = 0;
  $(document).on('ajaxSend.fundingScraper', () => { pageXhrInflight++; });
  $(document).on('ajaxComplete.fundingScraper', () => { pageXhrInflight--; });
